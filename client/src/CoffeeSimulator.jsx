...
const CoffeeSimulator = ({ userId: propUserId, selectedMenu }) => {
  ...
  const handleQTEClick = () => {
    ...
    if (pointerPosition >= targetLeft && pointerPosition <= targetRight) {
      setQteCount((prevCount) => prevCount + 1);
    } else {
  ...
  const handleFinishMenu = async () => {
    ...
    try {
      await updateUserAchievement(userId, menuId);
  ...
  return (
    <div className="relative" style={{ height: "90vh", width: "100%" }}>
      ...
      <div style={{ height: `${CONTENT_HEIGHT_PERCENT}vh`, width: "100%" }}>
        <div className="flex" style={{ height: "100%", width: "100%" }}>
          {/* Left Area - Equipment list (33.33%) */}
          <div
            className="p-2"
            style={{
              width: "33.33%",
          ...
          >
            <h3 className="text-center font-semibold text-2xl text-amber-900 mt-4">
              อุปกรณ์ที่ใช้ในเมนูนี้
            </h3>
            <div
          ...
          >
            ...
            <div
              className="flex-1 mt-4 flex flex-col items-center justify-start"
          ...
          {/* Right Area - Step List (ชิดขวาสุด) */}
          <div
            className="bg-orange-200 rounded shadow p-4 flex flex-col  justify-center"
            style={{
              width: "16.67%",
              height: "60%",
              margin: "auto 0",
      ...
      {/* Footer - คำอธิบาย */}
      <footer className="bg-gray-900 text-white p-3 flex items-center justify-center"
        style={{
          height: `${NAVBAR_HEIGHT_VH}vh`,
          width: "100%"
        }}
      >
        <p className="text-xl">
